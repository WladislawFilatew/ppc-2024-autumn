...
filatev_v_metod_zedela_mpi::MetodZedela::MetodZedela(std::shared_ptr<ppc::core::TaskData> taskData_) : Task(std::move(taskData_))
{
  if (world.rank() == 0) {
...
bool filatev_v_metod_zedela_mpi::MetodZedela::run() {
  ...
  boost::mpi::communicator new_comm = world.split(world.rank() != 0);
  ...
  if (world.rank() == 0){
    ...
    do {
      ...
      int start = world.rank() <= ost? (world.rank() - 1) * (delta + 1): ((delta + 1) * ost + (world.rank() - ost - 1) * delta);
...
void filatev_v_metod_zedela_mpi::MetodZedela::setAlfa(double _alfa){
  this->alfa = _alfa;
}

int filatev_v_metod_zedela_mpi::MetodZedela::rankMatrix(std::vector<int>& matrixT, int n) {
...
int filatev_v_metod_zedela_mpi::TestClassForMetodZedela::generatorVector(std::vector<int>& vec){
  int sum = 0;
...
void filatev_v_metod_zedela_mpi::TestClassForMetodZedela::generatorMatrix(std::vector<int>& matrix, int size){
  for (int i = 0; i < size; ++i) {
...
void filatev_v_metod_zedela_mpi::TestClassForMetodZedela::genetatirVectorB(std::vector<int>& matrix, std::vector<int>& vecB){
  int size = vecB.size();
...
void filatev_v_metod_zedela_mpi::TestClassForMetodZedela::coutSLU(std::vector<int> matrix, std::vector<int> vecB){
  int size = vecB.size();
  std::cout << "Matrix:\n";
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
...
bool filatev_v_metod_zedela_mpi::TestClassForMetodZedela::rightAns(std::vector<double>& answ, double alfa){
  double max_r = 0;
  for (int i = 0; i < answ.size(); ++i){
    max_r = std::max(max_r, abs(ans[i] - answ[i]));
    //std::cout << answ[i] << " " << ans[i] << std::endl;
  }
  return max_r < alfa;
}