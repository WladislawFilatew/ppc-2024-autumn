...
bool filatev_v_metod_zedela_mpi::SumMatrixSeq::pre_processing() {
  internal_order_test();
...
bool filatev_v_metod_zedela_mpi::SumMatrixSeq::validation() {
  internal_order_test();
  // Check count elements of output
  return taskData->inputs_count[0] > 0 && taskData->inputs_count[1] > 0 && taskData->outputs_count[0] == 1;
}

bool filatev_v_metod_zedela_mpi::SumMatrixSeq::run() {
  internal_order_test();
...
bool filatev_v_metod_zedela_mpi::SumMatrixSeq::post_processing() {
  internal_order_test();
...
bool filatev_v_metod_zedela_mpi::SumMatrixParallel::pre_processing() {
  internal_order_test();
  if (world.rank() == 0) {
    size_n = taskData->inputs_count[0];
    size_m = taskData->inputs_count[1];
...
bool filatev_v_metod_zedela_mpi::SumMatrixParallel::validation() {
  internal_order_test();
  if (world.rank() == 0) {
    // Check count elements of output
    return taskData->inputs_count[0] >= 0 && taskData->inputs_count[1] >= 0 && taskData->outputs_count[0] == 1;
...
bool filatev_v_metod_zedela_mpi::SumMatrixParallel::run() {
  internal_order_test();
  int delta = 0;
  int ras = 0;
...
bool filatev_v_metod_zedela_mpi::SumMatrixParallel::post_processing() {
  internal_order_test();
  if (world.rank() == 0) {
    reinterpret_cast<int*>(taskData->outputs[0])[0] = summ;
  }
